...
class DeviceModelUploadForm extends Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (scripts.length > 0 && scripts.some(({ validationResult }) => !validationResult )) {
      Observable.from(scripts)
        ...
        .flatMap(({ file }) => SimulationScriptsService
            .validateSimulationScript(file)
            .map(validationResult => ({file, validationResult}))
            .catch(error => {
              return Observable.of({file, validationResult: {
                success: false,
    ...
    this.readFileAsText(jsonFile)
      .then(content => {
        ...
        if (!isEqual(requiredScriptNames, uploadedScriptNames)) {
          ...
          this.setState({
            deviceModel,
            scripts: scripts.map(file => ({ file, validationResult: undefined }))
          });
        }
      })
      .catch(e => {
  ...
  apply = (event) => {
    ...
    Observable.from(scripts)
      .flatMap(({file}) => SimulationScriptsService
          .uploadsSimulationScript(file)
          .catch(error => console.log(error))
      )
      .reduce((scripts, script) => ({...scripts, [script.name]: script}), {})
      .map(scripts => {
        const { CloudToDeviceMethods = {} , Simulation: { Scripts } } = deviceModel;
