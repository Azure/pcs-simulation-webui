...
import { svgs, Validator, LinkedComponent } from 'utilities';
import {
  ErrorMsg,
  Btn,
  BtnToolbar,
...
export class DeviceJobMethods extends LinkedComponent {
  ...
  populateState(devices) {
    ...
    this.populateStateSubscription = Observable.from(devices)
      .map(({ cloudToDeviceMethods }) => new Set(Object.keys(cloudToDeviceMethods)))
  ...
  isFirmwareUpdate() {
    return this.methodNameLink.value
      ? this.methodNameLink.value === 'firmwareUpdate'
      : undefined;
  }
    ...
    return (
      <form onSubmit={this.apply}>
        <FormSection className="device-job-Methods-container">
          ...
          {error && <ErrorMsg>{error.errorMessage}</ErrorMsg>}
          {
            !changesApplied &&
            <BtnToolbar>
